...
class Aggregator(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, dropout, aggregator_type):
        ...
        if self.aggregator_type == 'gcn':
            ...
            # conv_dim_list 개수만큼 l1 l2 생김. layer 개수 3개 -> l1, l2 쌍 3개 나옴
    ...
    def forward(self, ego_embeddings, A_in):
        ...
        """
        # Equation (3) # side_embeddings.shape = (n_users + n_entities, in_dim)
        # side_embeddings의 의미: 라플라시안 행렬과 (유저, 아이템, 속성)에 대한 임베딩의 혼합 정보
...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.entity_user_embed = nn.Embedding(self.n_entities + self.n_users, self.embed_dim) # 얘는 학습됨
    ...
    def calc_cf_embeddings(self):
        ...
        for idx, layer in enumerate(self.aggregator_layers):
            # ego_embed 첫 iteration: 1-hop 정도 획득, 2번째 iteration: ego_embed에 1-hop 정보가 묻혀진 상태로 A_in 행렬과 연산하면서 2-hop 정보 획득한다고 간주.
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        # item_pos_embed, item_neg_embed, user_embed -> 모두 1-hop, 2-hop, ... l-hop 정보가 concatenate되어 있음. 따라서 모든 layer에 대해 l2 loss를 한 것임.
    ...
    def calc_kg_loss(self, h, r, pos_t, neg_t):
        ...
        # Equation (1) TransR
        # pos_score, neg_score: for a given triplet (h,r,t), its plausibility score (or energy score)
        # A lower score suggests that the triplet is more likely to be true true, and vice versa.
        # KG 임베딩
        pos_score = torch.sum(torch.pow(r_mul_h + r_embed - r_mul_pos_t, 2), dim=1)     # (kg_batch_size)
        neg_score = torch.sum(torch.pow(r_mul_h + r_embed - r_mul_neg_t, 2), dim=1)     # (kg_batch_size)
        # Equation (2) pairwise ranking loss
    ...
    def update_attention(self, h_list, t_list, r_list, relations):
        ...
        for r_idx in relations:
            # torch.where(조건식, True 시 반환할 값, False 시 반환할 값)
            # 조건식에 Tensor를 포함해야함. 해당 Tensor에서의 index가 True인 인덱스들을 반환함.
            # torch.where는 조건에 부합하는 값들을 다른 값으로 바꾸고 싶을 때 함.
            index_list = torch.where(r_list == r_idx)
            batch_h_list = h_list[index_list]
            batch_t_list = t_list[index_list]

            # r_idx번째 relation에 대응되는 head, tail -> head랑 tail같의 attention score
